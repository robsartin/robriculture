...
- `python -m harness.promotion --succeed <challenger> --issue N --pr N --wins W --games G --ties T --seeds A-B --date YYYY-MM-DD` — designate a challenger that PROMOTED against the recorded `gate_opponent`; writes both roles to `harness/champion.json` as a **gate succession** (#241, ADR-0007 amendment 2026-09-07). The committed artifact was produced this way.
- `python -m harness.promotion --designate --games 2 [--include-external]` — rank all strategies by **pool share** against the fixed anchors (#76), with the pinned gate externals in the pool and the candidates on `--include-external` (#152). Informational since #241: it prints the ranking and does not overwrite a succession artifact. `--games` here means games *per pairing* (candidates x 6 anchors), not total seeded games.
- `python -m scripts.fetch_external_agents [--pin]` — fetch the licence-checked external agents named in `harness/external_agents.json` into the gitignored `external_agents/`; a pinned entry is verified by sha256 and refused on a mismatch, `--pin` records the hash (and the GitHub commit) of an unpinned entry (#152). The gate's external limb will not run without a verified pool.
- `python -m harness.rounds --games 20` — play a round, append it to `harness/rounds.json`, and rank by pool share (#12, #76); it cannot overwrite a succession artifact either.
...
- **Beat the *designated gate opponent*, not an assumption.** `harness/champion.json` records two roles (#76): `gate_opponent` — what an ADR-0007 experiment is measured against, which is always ours (the last challenger to PROMOTE, #241); the field enters through the gate's paired external limb against `external_pool.EXTERNAL_ANCHORS` (#152) — and `submit_default`, what `scripts/submit.py` packages, which is **never** a benchmark (submitting a vendored competitor's agent is an ADR-0005 licensing problem). Designation is by **gate succession** (#241): the champion is the most recent strategy to PROMOTE through the ADR-0007 gate against the incumbent, and the artifact records that succession. Pool share (#76) is a ranking only — once every contender swept the anchors it put a gate-REJECTED contender first.
