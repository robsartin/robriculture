"""fert_sixteen's third run (#329): sixteen_bench's controls, criterion and arm B
on fresh seeds, under the paired external limb as amended 2026-09-20 (PR #328).

#324 and #326 each read champion 15/16, anchors 16/16 and a one-game
shortfall on one external beside a large lonespear gain, and were rejected
by the per-pair limb of their day. The pairs are now judged as a whole
(ADR-0007, 2026-09-20). This is a new run on fresh seeds, not a re-read.

    .venv/bin/python -m harness.sixteenc_bench --controls
    .venv/bin/python -m harness.sixteenc_bench --criterion
    .venv/bin/python -m harness.sixteenc_bench --recorded
"""
...
def main(argv=None):  # pragma: no cover
    os.environ.setdefault("ROBRICULTURE_STRICT", "1")
    ap = argparse.ArgumentParser(description="fert_sixteen, third run on fresh seeds")
