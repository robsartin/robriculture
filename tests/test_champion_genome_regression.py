...
`strategies/champion_genome.json`'s `meta.share`/`meta.win_rate` first:
  ...
  - #71's own multi-seed evolution run (10 seeds x 25 generations)
    ...
    side effect, the new genome's decoded land-reserve threshold moved:
    it now issues `BUY_LAND` at the `land_reserve_below` money value
    too (previously it did not), so that scenario and
    `land_reserve_above` produce identical actions. See the comment on
    `test_no_buy_land_when_money_just_below_reserve_threshold` below.
"""
...
def test_no_buy_land_when_money_just_below_reserve_threshold():
    """Frozen action set at the `land_reserve_below` money value.

    This scenario's dollar amount was originally pinned just *under* the
    prior champion's land-purchase reserve threshold. The #71 promotion
    (2026-08-24, share 0.5222) evolved a genome whose decoded reserve is
    lower, so this same money value now clears it too -- the golden
    action set includes `BUY_LAND` here, identically to
    `land_reserve_above`. This is a legitimate consequence of the new
    genome (see the module docstring's re-blessing note), not a wiring
    regression: nothing pins a below-threshold boundary for the new
    genome at this specific dollar value any more.
    """
