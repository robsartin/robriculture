...
def test_strategy_completes_without_crashing(name, opponent):
    """Every registered strategy survives a full season, and clears the sanity
    floor against `random`.

    Both assertions read the same games. They used to be two tests playing the
    `random` pairing twice over the same three seeds -- 78 redundant 720-turn
    simulations, a third of this file's cost, for no extra coverage.
    """
    ...
    # Sanity floor: each strategy should out-earn 'random' on average. Held
    # against `random` only -- `starter` is a real agent and beating it is not a
    # floor any strategy is required to clear.
    if opponent == "random":
