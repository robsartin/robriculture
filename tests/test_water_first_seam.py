...
def _melon(planted, yield_units, watered=False):
...
def test_plot_action_waters_a_ready_unwatered_melon_below_full_yield_only_when_asked():
    ...
    assert fr.plot_action(_melon(0, 5, watered=True), "MELON", 10, 5, water_first=True) == ["HARVEST"]
    assert fr.plot_action(_melon(0, 6), "MELON", 10, 5, water_first=True) == ["HARVEST"]       # already full
    assert fr.plot_action(_melon(0, 5), "MELON", 13, 5, water_first=True) == ["HARVEST"]       # past max_yield_day
    assert fr.plot_action(_melon(0, 4), "MELON", 9, 5, water_first=True) == ["WATER"]           # not ready: frozen waters anyway
...
def test_crop_worker_banks_at_the_given_carry():
    cluster = [(1, 1)]
    board = _cluster_board({(1, 1): _melon(0, 6, watered=True)})
    pos = [1, 1]
    heavy = {"MELON": 6}
    walk_to_shed = hh.step_toward(pos, fr.nearest_shed(pos))
    assert fr.crop_worker_action(cluster, board, pos, heavy, "MELON", 10, 5) == walk_to_shed             # frozen: 6 banks
    assert fr.crop_worker_action(cluster, board, pos, heavy, "MELON", 10, 5, carry=18) == ["HARVEST"]
    assert fr.crop_worker_action(cluster, board, pos, {"MELON": 18}, "MELON", 10, 5, carry=18) == walk_to_shed


def test_crop_worker_passes_water_first_through():
    cluster = [(1, 1)]
    board = _cluster_board({(1, 1): _melon(0, 5)})
    pos = [1, 1]
